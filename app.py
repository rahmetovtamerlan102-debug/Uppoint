...
# ---------- HELPER: обрезаем пароль до 72 байт ----------
def truncate_password(password: str) -> str:
    """Обрезает пароль до 72 байт (требование bcrypt)"""
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        ...
        # Превращаем список регионов в строку для PostgreSQL массива
        ...
        if not demo:
            demo_password = truncate_password("demo123")
            await conn.execute(
                "INSERT INTO users (email, password_hash) VALUES ($1, $2)",
                "demo@uppoint.com", pwd_context.hash(demo_password)
            )

        # Создаём партиции на будущие месяцы
...
async def login(data: LoginRequest, request: Request):
    async with db_pool.acquire() as conn:
        ...
        if not user or not pwd_context.verify(truncate_password(data.password), user["password_hash"]):
...
async def register(data: RegisterRequest, request: Request):
    async with db_pool.acquire() as conn:
        ...
        truncated_pwd = truncate_password(data.password)
        pwd_hash = pwd_context.hash(truncated_pwd)
...
async def status_page():
    return HTMLResponse('<html><body><h1>Public Status</h1><div id="status"></div><script>fetch("/api/status").then(r=>r.json()).then(data=>{document.getElementById("status").innerHTML=data.monitors.map(m=>`<div><b>${m.name}</b> ${m.status.toUpperCase()} ${m.response_ms}ms</div>`).join("")});setInterval(()=>location.reload(),30000);</script></body></html>')

@app.get("/logout")
async def logout():
    return HTMLResponse('<script>localStorage.removeItem("token");location.href="/";</script>')
